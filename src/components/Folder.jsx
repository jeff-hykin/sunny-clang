...
import { VscChevronRight, VscChevronDown, VscFolder, VscFile, VscNewFolder, VscNewFile, VscEdit, VscTrash } from "https://esm.sh/react-icons/vsc?target=esnext&dev&deps=react@17.0.1,react-dom@17.0.1"

export const Folder = ({ handleInsertNode, handleDeleteNode, handleUpdateFolder, explorerData }) => {
    const [nodeName, setNodeName] = useState(explorerData?.name ? explorerData.name : "")
            ...
            <div>
                <div className="folder" style={{ cursor: "pointer" }} onClick={() => setExpand(!expand)}>
                    <span>
                        ...
                        {updateInput.visible ? <input type="text" value={nodeName} onChange={handleChange} autoFocus onBlur={() => setUpdateInput({ ...updateInput, visible: false })} onKeyDown={onUpdate} /> : <label>{explorerData.name}</label>}
                    </span>
                ...
                <div id="folderContainer" style={{ display: expand ? "block" : "none", marginLeft: 20 }}>
                    {showInput.visible && (
                        <div className="addItem">
                            ...
                            <input type="text" autoFocus onBlur={() => setShowInput({ ...showInput, visible: false })} onKeyDown={onAdd} />
                        </div>
                    )}
                    {explorerData.items.map((item, index) => {
                        return <Folder handleDeleteNode={handleDeleteNode} handleInsertNode={handleInsertNode} handleUpdateFolder={handleUpdateFolder} explorerData={item} key={index} />
                    })}
                </div>
            </div>
        )
